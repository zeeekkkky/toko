// checkout.js - Complete Checkout System with WhatsApp Integration (FIXED)
...
// Send order to WhatsApp - FIXED VERSION
function sendToWhatsApp(orderData) {
    ...
    // Format items list - SIMPLIFIED
    const itemsList = orderData.items.map(item => 
        `${item.name} - ${item.quantity}x @ Rp ${item.price.toLocaleString('id-ID')}`
    ).join('%0A');
    
    // Create WhatsApp message - SIMPLIFIED & SHORTER
    const message = `_*Nota Pemesanan Toko Zek*_%0A%0A` +
        `No. Pesanan : ${orderData.orderId}%0A` +
        `Tanggal : ${orderData.orderDate}%0A%0A` +
        
        `_*Data Penerima :*_%0A` +
        `Nama : ${orderData.personalInfo.fullName}%0A` +
        `Telepon : ${orderData.personalInfo.phone}%0A` +
        `Email : ${orderData.personalInfo.email}%0A%0A` +
        
        `_*Alamat Pengiriman :*_%0A` +
        `${orderData.shipping.address}%0A` +
        `${orderData.shipping.city}, ${orderData.shipping.province} ${orderData.shipping.postalCode}%0A%0A` +
        
        `_*Daftar Produk :*_%0A` +
        `${itemsList}%0A%0A` +
        
        `_*Rincian Pembayaran :*_%0A` +
        `Subtotal: Rp ${orderData.subtotal.toLocaleString('id-ID')}%0A` +
        `Pengiriman : (${shippingMethodText}): Rp ${orderData.shippingCost.toLocaleString('id-ID')}%0A` +
        `_*Total : Rp ${orderData.total.toLocaleString('id-ID')}*_%0A%0A` +
        
        `_*Metode Pembayaran :*_%0A` +
        `${orderData.payment.methodName}%0A%0A` +
        
        `_*Mohon Menunggu Respon Penjual !*_`;
    
    // Create WhatsApp URL - USING wa.me
    const phoneNumber = '6282288691614';
    const waURL = `https://wa.me/${phoneNumber}?text=${message}`;
    
    console.log('WhatsApp URL:', waURL); // For debugging
...
function showSuccessPopup(orderData) {
    ...
    popup.innerHTML = `
        <div class="success-content">
            ...
            <div style="display: flex; gap: 1rem; margin-top: 2rem;">
                <button id="whatsappBtn" class="btn-secondary" style="flex: 1; padding: 1rem; border: 2px solid #25D366; background: white; color: #25D366; border-radius: 25px; font-weight: 600; cursor: pointer; font-family: 'Poppins', sans-serif; transition: all 0.3s ease;">
                    <i class="fab fa-whatsapp"></i> Pesan via WhatsApp
                </button>
                <button onclick="window.location.href='index.html'" class="btn-primary" style="flex: 1; padding: 1rem; border: none; background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); color: white; border-radius: 25px; font-weight: 600; cursor: pointer; font-family: 'Poppins', sans-serif; transition: all 0.3s ease;">
    ...
    // Add event listener to WhatsApp button - CRITICAL FIX
    setTimeout(() => {
